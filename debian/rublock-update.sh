...
for chain in PREROUTING OUTPUT; do
    while iptables -t nat -D "$chain" -p tcp -m set --match-set rublack-dns dst -j REDIRECT --to-ports 9040 2>/dev/null; do :; done
    while iptables -t nat -D "$chain" -p tcp -m set --match-set rublack-ip dst -j REDIRECT --to-ports 9040 2>/dev/null; do :; done
    while iptables -t nat -D "$chain" -p udp -m set --match-set rublack-dns dst -j REDIRECT --to-ports 9040 2>/dev/null; do :; done
    while iptables -t nat -D "$chain" -p udp -m set --match-set rublack-ip dst -j REDIRECT --to-ports 9040 2>/dev/null; do :; done
done
...
# Создание rublack-dns если не существует
...
RULES_COUNT=$(iptables -t nat -L PREROUTING -n | grep -c rublack || echo 0)
