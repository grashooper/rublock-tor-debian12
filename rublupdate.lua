Что-то было потеряно в коде
Проверь ещё раз учитывая всё выше и предоставь полный правильный код
...
-- "degrade" - использовать доступные источники
-- "fail" - падать если хотя бы один недоступен
...
local timer = {
start_time = nil,
start = function(self) self.start_time = os.time() end,
...
if seconds < 60 then return string.format("%.1fs", seconds)
elseif seconds < 3600 then return string.format("%dm %ds", math.floor(seconds / 60), seconds % 60)
...
local cmd = string.format("timeout %d curl -fsSL --compressed --max-time %d '%s' 2>/dev/null", timeout, timeout, url)
...
local cmd = string.format("timeout %d bash -c "curl -fsSL --max-time %d '%s' 2>/dev/null | gunzip -c 2>/dev/null"", timeout, timeout, url)
local handle = io.popen(cmd, "r")
...
ip = ip:match("^%s*(.-)%s*")ifnotiporip==""thenreturnnilendip=ip:gsub("/32")ifnotiporip==""thenreturnnilendip=ip:gsub("/32", "")
if is_valid_ipv4(ip) or is_valid_subnet(ip) then return ip end
return nil
end
...
elseif (n < 2048) then return string.char(192 + ((n - (n % 64)) / 64), 128 + (n % 64))
else return string.char(224 + ((n - (n % 4096)) / 4096), 128 + (((n % 4096) - (n % 64)) / 64), 128 + (n % 64))
end
end
...
if chunk == nil then streamEnded = true else buffer = buffer .. chunk end
...
elseif escapedChar == "u" then currentRecord = currentRecord .. "\u"
else currentRecord = currentRecord .. escapedChar
end
else
currentRecord = currentRecord .. buffer:sub(bufferPos, #buffer)
buffer, bufferPos = "", 1
if streamEnded then return currentRecord ~= "" and currentRecord or nil end
break
end
end
end
...
if http_fetch(config.urls.antifilter_fqdn, ltn12.sink.table(buf), timeout) then
    ...
    if cached then
        ...
        print_info("💾", string.format("Loaded from cache (%s, age: %dd)", format_number(stats.domains), math.floor((age or 0) / 86400)))
    else
...
if http_fetch(config.urls.antifilter_net, ltn12.sink.table(buf), timeout) then
    ...
    print_success("✓", string.format("Subnets: %s (CIDR notation, matched by range)", format_number(stats.subnets)))
else
...
for line in data:gmatch("[^\r\n]+") do
    ...
    if lineCount % 50000 == 0 then print_progress(lineCount, totalLines, "Processing") end
...
print_success("✓", string.format("Domains: %s, IPs: %s", format_number(stats.domains), format_number(stats.ips)))
...
if http_fetch(config.urls.antizapret_domains, extractSink, timeout) then
    print(string.format("\r  Processed: %s domains                              ", format_number(domain_count)))
    ...
    for sld in pairs(subdomainsCount) do
        if config.neverGroupDomains[sld] then
            ...
            for _, pattern in ipairs(config.neverGroupMasks) do
                if sld:find(pattern) then subdomainsCount[sld] = 0; break end
            end
        end
    end
end
...
for fqdn, data in pairs(fqdnList) do
    ...
    if (not fqdnList[sld]) or (fqdn == sld) then
        ...
        if not domainTable[keyValue] then domainTable[keyValue] = data.sources end
    end
end

if grouped > 0 then print_success("✓", string.format("Grouped %s SLDs", format_number(grouped))) end
return domainTable
...
for fqdn in pairs(domainList) do
    ...
    count = count + 1
    if count % 50000 == 0 then print_progress(count, total, "Writing domains") end
end

if total > 0 then print_progress(total, total, "Writing domains") end
configFile:close()
...
for ipaddr in pairs(ipList) do
    ...
    count = count + 1
    if ipaddr:match("/") then subnets_count = subnets_count + 1 end
    if count % 50000 == 0 then print_progress(count, total, "Writing IPs") end
end

if total > 0 then print_progress(total, total, "Writing IPs") end
...
print(string.format(" %sUnique domains:%s %s%s%s", colors.dim, colors.reset, colors.white, format_number(table_count(bltables.fqdn)), colors.reset))
print(string.format(" %sUnique IPs/subnets:%s %s%s%s", colors.dim, colors.reset, colors.white, format_number(table_count(bltables.ips)), colors.reset))

text

local source_stats = {}
for _, data in pairs(bltables.fqdn) do
    for _, src in ipairs(data.sources) do source_stats[src] = (source_stats[src] or 0) + 1 end
end
...
for _, data in pairs(bltables.ips) do
    for _, src in ipairs(data.sources) do ip_stats[src] = (ip_stats[src] or 0) + 1 end
end
...
print_header("rublock-tor Multi-Source Updater v3.2")
