...
**Автоматическая маршрутизация трафика через Tor. Разработано специально для Debian 12 (Bookworm).**

Система прозрачно маршрутизирует трафик через сеть Tor, сохраняя прямое соединение и максимальную скорость для обычных сайтов.
...
- 🚀 **Высокая производительность**: Обычный трафик (YouTube, игры, загрузки) идет напрямую без замедления. Через Tor проходит определенный трафик.
...
git clone https://github.com/grashooper/tor-gateway-debian12.git
cd privacy-tor-gateway-debian12/debian
...
4. Скачает списки ресурсов и запустит сервисы.
...
| Действие | Команда |
|:---|:---|
| **Обновить списки вручную** | `sudo tor-gateway-update.sh` |
| **Проверить статус Tor** | `systemctl status tor` |
| **Проверить статус DNS** | `systemctl status dnsmasq` |
| **Посмотреть таймер** | `systemctl list-timers tor-gateway-update.timer` |
| **Логи обновления** | `journalctl -u tor-gateway-update.service -n 50` |
